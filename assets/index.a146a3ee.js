...
`,M="/probe-match-verifier/assets/color-spectrum.97f3c94a.png";const P="probe-match-verifier",Z="0.0.0",I="Probe Match Verifier",U="A probe match verification tool for ColourSpace.",D={start:"vite",clean:"rimraf dist coverage",build:"pnpm run clean && tsc && vite build","build:gh-pages":"pnpm run build -- --base=/probe-match-verifier/",preview:"vite preview",prepreview:"pnpm run build",lint:"run-p lint:*","lint:eslint":"eslint --ext .js,.ts .","lint:prettier":"prettier '**/*.{js,ts,md,css}' --check","lint:types":"tsc --noEmit",format:"run-s format:*","format:eslint":"eslint --ext .js,.ts . --fix","format:prettier":"prettier '**/*.{js,ts,md,css}' --write",test:"vitest run","test:watch":"vitest",coverage:"vitest --coverage",preinstall:"npx -y only-allow pnpm",postinstall:"rimraf 'public/shoelace' && cpy 'node_modules/@shoelace-style/shoelace/dist/assets/icons' 'public/shoelace/assets/icons'"},S=["colourspace","lightillusion","probe","matching","verification","spectro"],N="Peter Siska <63866+peschee@users.noreply.github.com>",H="MIT",O={"@types/node":"^17.0.6","@typescript-eslint/eslint-plugin":"^5.8.1","@typescript-eslint/parser":"^5.8.1",c8:"^7.11.0","cpy-cli":"^3.1.1",eslint:"^8.6.0","eslint-config-prettier":"^8.3.0","eslint-plugin-lit":"^1.6.1","eslint-plugin-wc":"^1.3.2","lint-staged":"^12.1.4","npm-run-all":"^4.1.5",prettier:"^2.5.1","remark-gfm":"^3.0.1",rimraf:"^3.0.2","simple-git-hooks":"^2.7.0","ts-lit-plugin":"^1.2.1",typescript:"^4.5.4",vite:"^2.7.10","vite-plugin-html":"^2.1.2","vite-remark-html":"^1.1.1",vitest:"^0.0.127"},j={"@shoelace-style/shoelace":"^2.0.0-beta.63","@vanillawc/wc-markdown":"^1.6.11","browser-fs-access":"^0.23.0","cie-colorconverter":"^2.0.1","fast-xml-parser":"^4.0.0-beta.8",lit:"^2.0.2","normalize.css":"^8.0.1"};var Y={name:P,version:Z,title:I,description:U,private:!0,scripts:D,keywords:S,author:N,license:H,devDependencies:O,dependencies:j,"lint-staged":{"**/*.{js,ts,md,css}":["prettier --write"],"**/*.{js,ts}":["eslint --fix"]},"simple-git-hooks":{"pre-commit":"pnpx lint-staged","pre-push":"CI=1 pnpm run lint && pnpm test"}},V=Object.defineProperty,A=Object.getOwnPropertyDescriptor,p=(e,t,r,a)=>{for(var s=a>1?void 0:a?A(t,r):t,i=e.length-1,l;i>=0;i--)(l=e[i])&&(s=(a?l(t,r,s):l(s))||s);return a&&s&&V(t,r,s),s};W("/probe-match-verifier/shoelace");const w=!1;let o=class extends R{constructor(){super();this.csXML=new L({debug:w}),this.updateHeaderSize=this.updateHeaderSize.bind(this)}async connectedCallback(){super.connectedCallback(),console.info(`Version: ${this.version}`),window.addEventListener("resize",this.updateHeaderSize)}disconnectedCallback(){window.removeEventListener("resize",this.updateHeaderSize),super.disconnectedCallback()}render(){return c`
      ...
      <footer>
        <sl-dialog data-dialog-about class="about-dialog" @sl-hide="${()=>o.trackUmamiEvent("close","about")}">
    ...
    `}async updated(e){super.updated(e),this.updateHeaderSize(),e.has("referenceBpd")&&this.referenceBpd&&(this.referenceRGBW=this.csXML.getRGBWFromBpd(await this.referenceBpd.text()),this.requestUpdate("referenceRGBW")),e.has("verificationBcs")&&this.verificationBcs&&(this.verificationRGBW=this.csXML.getRGBWFromBcs(await this.verificationBcs.text()),this.requestUpdate("verificationRGBW")),(e.has("referenceRGBW")||e.has("verificationRGBW"))&&(this.xyYErrors=this.computexyYErrors(this.referenceRGBW,this.verificationRGBW),this.requestUpdate("xyYErrors"))}async openVerificationBcsFile(){o.trackUmamiEvent("open","bcs");try{const e=await B({description:"BCS files",extensions:[".bcs"]});this.verificationBcs=e,this.requestUpdate("verificationBcs"),o.trackUmamiEvent("load","bcs")}catch(e){console.error(e),o.trackUmamiEvent(`error: ${e}`,"bcs")}}async openReferenceBpdFile(){o.trackUmamiEvent("open","bpd");try{const e=await B({description:"BPD files",extensions:[".bpd"]});this.referenceBpd=e,this.requestUpdate("referenceBpd"),o.trackUmamiEvent("load","bpd")}catch(e){console.error(e),o.trackUmamiEvent(`error: ${e}`,"bpd")}}renderRGBW(e){return e.length<=0?h:c`
      <table>
        ...
        ${e.map((t,r)=>c` <tr>
              <td>${o.getRowLabelForIndex(r)}</td>
              ${t.map((a,s)=>c` <td
                    class="${k({red:s===this.csXML.getRGBWIndexForColor("red"),green:s===this.csXML.getRGBWIndexForColor("green"),blue:s===this.csXML.getRGBWIndexForColor("blue"),white:s===this.csXML.getRGBWIndexForColor("white")})}"
                  >
                    ${d(a,7)}
                  </td>`)}
            </tr>`)}
      </table>
    `}renderComparisonTable(e,t,r){return o.trackUmamiEvent("render","comparison"),c`
    ...
    `}renderComparisonForColor(e,t,r,a){const s=this.csXML.getRGBWIndexForColor(a),i={x:e[0][s],y:e[1][s],Y:e[2][s]},l={x:t[0][s],y:t[1][s],Y:t[2][s]};return c`
      ...
      <tr class="${a}">
        ...
        <td>${d(i.x,7)}</td>
        <td>${d(l.x,7)}</td>
        <td class="align-percent">${o.renderError(r[a].x)}</td>
        <td class="clear">${o.renderNistPassFail(o.passesNistxy(r[a].x))}</td>
      </tr>
      <tr class="${a}">
        <th>y</th>
        <td>${d(i.y,7)}</td>
        <td>${d(l.y,7)}</td>
        <td class="align-percent">${o.renderError(r[a].y)}</td>
        <td class="clear">${o.renderNistPassFail(o.passesNistxy(r[a].y))}</td>
      </tr>
      <tr class="${a}">
        <th>Y</th>
        <td>${d(i.Y,7)}</td>
        <td>${d(l.Y,7)}</td>
        <td class="number-percent">${o.renderError(r[a].Y)}</td>
        <td class="clear">${o.renderNistPassFail(o.passesNistY(r[a].Y))}</td>
      </tr>
    `}static getRowLabelForIndex(e){const t=["x","y","Y"];return typeof t[e]=="undefined"?"n/a":t[e]}computexyYErrors(e=[],t=[]){if(e.length<1||t.length<1)return;const r=this.csXML.getRGBWIndexForColor("red"),a=this.csXML.getRGBWIndexForColor("green"),s=this.csXML.getRGBWIndexForColor("blue"),i=this.csXML.getRGBWIndexForColor("white"),l={x:t[0][r]-e[0][r],y:t[1][r]-e[1][r],Y:f(e[2][r],t[2][r])},g={x:t[0][a]-e[0][a],y:t[1][a]-e[1][a],Y:f(e[2][a],t[2][a])},u={x:t[0][s]-e[0][s],y:t[1][s]-e[1][s],Y:f(e[2][s],t[2][s])},m={x:t[0][i]-e[0][i],y:t[1][i]-e[1][i],Y:f(e[2][i],t[2][i])};return{red:l,green:g,blue:u,white:m}}static passesNistxy(e){return Math.abs(e)<=.001}static passesNistY(e){return Math.abs(e)<=1.5}static renderNistPassFail(e){return e?c`<span class="text-success">Pass</span>`:c`<span class="text-error">Fail</span>`}static renderError(e){return d(e,7)}updateHeaderSize(){!this.appHeader||!this.appContent||this.appContent.style.setProperty("--app-header-size",`${this.appHeader.offsetHeight}px`)}static trackUmamiEvent(e,t="custom"){const r=window.umami;!r||r.trackEvent(e,t)}};o.styles=F(z);p([C()],o.prototype,"version",2);p([b()],o.prototype,"referenceBpd",2);p([b()],o.prototype,"verificationBcs",2);p([b()],o.prototype,"referenceRGBW",2);p([b()],o.prototype,"verificationRGBW",2);p([b()],o.prototype,"xyYErrors",2);p([y("[data-dialog-about]")],o.prototype,"aboutDialog",2);p([y("[data-app-header]")],o.prototype,"appHeader",2);p([y("[data-app-content]")],o.prototype,"appContent",2);o=p([G("app-shell")],o);
