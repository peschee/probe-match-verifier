...
`,V=Object.defineProperty,O=Object.getOwnPropertyDescriptor,p=(e,t,a,r)=>{for(var s=r>1?void 0:r?O(t,a):t,i=e.length-1,n;i>=0;i--)(n=e[i])&&(s=(r?n(t,a,s):n(s))||s);return r&&s&&V(t,a,s),s};M("/probe-match-verifier/shoelace");const C=!1;let l=class extends ${constructor(){super();this.csXML=new E({debug:C}),this.updateHeaderSize=this.updateHeaderSize.bind(this)}async connectedCallback(){super.connectedCallback(),window.addEventListener("resize",this.updateHeaderSize)}disconnectedCallback(){window.removeEventListener("resize",this.updateHeaderSize),super.disconnectedCallback()}render(){return c`
    ...
    `}renderComparisonTable(e,t,a){return c`
      <table class="profile-comparison">
        ${this.renderComparisonForColor(e,t,a,"red")} ${this.renderComparisonForColor(e,t,a,"green")}
        ${this.renderComparisonForColor(e,t,a,"blue")} ${this.renderComparisonForColor(e,t,a,"white")}
      </table>
    `}renderComparisonForColor(e,t,a,r){const s=this.csXML.getRGBWIndexForColor(r),i={x:e[0][s],y:e[1][s],Y:e[2][s]},n={x:t[0][s],y:t[1][s],Y:t[2][s]};return c`
      ...
      <tr class="${r}">
        ...
        <td>${d(i.x)}</td>
        <td>${d(n.x)}</td>
        <td class="align-percent">${l.renderError(a[r].x)}</td>
        <td class="clear">${l.renderNistPassFail(l.passesNistxy(a[r].x))}</td>
      </tr>
      <tr class="${r}">
        <th>y</th>
        <td>${d(i.y)}</td>
        <td>${d(n.y)}</td>
        <td class="align-percent">${l.renderError(a[r].y)}</td>
        <td class="clear">${l.renderNistPassFail(l.passesNistxy(a[r].y))}</td>
      </tr>
      <tr class="${r}">
        <th>Y</th>
        <td>${d(i.Y)}</td>
        <td>${d(n.Y)}</td>
        <td class="number-percent">${l.renderError(a[r].Y)}</td>
        <td class="clear">${l.renderNistPassFail(l.passesNistY(a[r].Y))}</td>
      </tr>
    `}static getRowLabelForIndex(e){const t=["x","y","Y"];return typeof t[e]=="undefined"?"n/a":t[e]}computexyYErrors(e=[],t=[]){if(e.length<1||t.length<1)return;const a=this.csXML.getRGBWIndexForColor("red"),r=this.csXML.getRGBWIndexForColor("green"),s=this.csXML.getRGBWIndexForColor("blue"),i=this.csXML.getRGBWIndexForColor("white"),n={x:t[0][a]-e[0][a],y:t[1][a]-e[1][a],Y:f(e[2][a],t[2][a])},u={x:t[0][r]-e[0][r],y:t[1][r]-e[1][r],Y:f(e[2][r],t[2][r])},g={x:t[0][s]-e[0][s],y:t[1][s]-e[1][s],Y:f(e[2][s],t[2][s])},m={x:t[0][i]-e[0][i],y:t[1][i]-e[1][i],Y:f(e[2][i],t[2][i])};return{red:n,green:u,blue:g,white:m}}static passesNistxy(e){return Math.abs(e)<=.001}static passesNistY(e){return Math.abs(e)<=1.5}static renderNistPassFail(e){return e?c`<span class="text-success">Pass</span>`:c`<span class="text-error">Fail</span>`}static renderError(e){return d(e)}updateHeaderSize(){!this.appHeader||!this.appContent||this.appContent.style.setProperty("--app-header-size",`${this.appHeader.offsetHeight}px`)}};l.styles=F(z);p([b()],l.prototype,"referenceBpd",2);p([b()],l.prototype,"verificationBcs",2);p([b()],l.prototype,"referenceRGBW",2);p([b()],l.prototype,"verificationRGBW",2);p([b()],l.prototype,"xyYErrors",2);p([y("[data-dialog-about]")],l.prototype,"aboutDialog",2);p([y("[data-app-header]")],l.prototype,"appHeader",2);p([y("[data-app-content]")],l.prototype,"appContent",2);l=p([G("app-shell")],l);
