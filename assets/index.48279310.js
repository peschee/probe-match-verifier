...
`,O=Object.defineProperty,j=Object.getOwnPropertyDescriptor,p=(e,t,o,r)=>{for(var s=r>1?void 0:r?j(t,o):t,i=e.length-1,l;i>=0;i--)(l=e[i])&&(s=(r?l(t,o,s):l(s))||s);return r&&s&&O(t,o,s),s};X("/probe-match-verifier/shoelace");const B=!1;let n=class extends ${constructor(){super();this.csXML=new E({debug:B})}async connectedCallback(){super.connectedCallback()}render(){return c`
    ...
    `}renderComparisonForColor(e,t,o,r){const s=this.csXML.getRGBWIndexForColor(r),i={x:e[0][s],y:e[1][s],Y:e[2][s]},l={x:t[0][s],y:t[1][s],Y:t[2][s]};return c`
      ...
      <tr class="${r}">
        ...
        <td>${d(i.x)}</td>
        <td>${d(l.x)}</td>
        <td class="align-percent">${n.renderError(o[r].x)}</td>
        <td class="clear">${n.renderNistPassFail(n.passesNistxy(o[r].x))}</td>
      </tr>
      <tr class="${r}">
        <th>y</th>
        <td>${d(i.y)}</td>
        <td>${d(l.y)}</td>
        <td class="align-percent">${n.renderError(o[r].y)}</td>
        <td class="clear">${n.renderNistPassFail(n.passesNistxy(o[r].y))}</td>
      </tr>
      <tr class="${r}">
        <th>Y</th>
        <td>${d(i.Y)}</td>
        <td>${d(l.Y)}</td>
        <td class="number-percent">${n.renderError(o[r].Y)}</td>
        <td class="clear">${n.renderNistPassFail(n.passesNistY(o[r].Y))}</td>
      </tr>
    `}static getRowLabelForIndex(e){const t=["x","y","Y"];return typeof t[e]=="undefined"?"n/a":t[e]}computexyYErrors(e=[],t=[]){if(e.length<1||t.length<1)return;const o=this.csXML.getRGBWIndexForColor("red"),r=this.csXML.getRGBWIndexForColor("green"),s=this.csXML.getRGBWIndexForColor("blue"),i=this.csXML.getRGBWIndexForColor("white"),l={x:e[0][o]-t[0][o],y:e[1][o]-t[1][o],Y:b(e[2][o],t[2][o])},u={x:e[0][r]-t[0][r],y:e[1][r]-t[1][r],Y:b(e[2][r],t[2][r])},g={x:e[0][s]-t[0][s],y:e[1][s]-t[1][s],Y:b(e[2][s],t[2][s])},m={x:e[0][i]-t[0][i],y:e[1][i]-t[1][i],Y:b(e[2][i],t[2][i])};return{red:l,green:u,blue:g,white:m}}static passesNistxy(e){return Math.abs(e)<=.001}static passesNistY(e){return Math.abs(e)<=1.5}static renderNistPassFail(e){return e?c`<span class="text-success">Pass</span>`:c`<span class="text-error">Fail</span>`}static renderError(e){return d(e)}};n.styles=F(P);p([f()],n.prototype,"referenceBpd",2);p([f()],n.prototype,"verificationBcs",2);p([f()],n.prototype,"referenceRGBW",2);p([f()],n.prototype,"verificationRGBW",2);p([f()],n.prototype,"xyYErrors",2);p([C("[data-dialog-about]")],n.prototype,"aboutDialog",2);n=p([L("app-shell")],n);
